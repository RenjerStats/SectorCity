...
/// доле объёма папки — см. [`AggSpec`]). До первого скана отдаём мок.
#[tauri::command]
...
/// Глобальный поиск по имени в текущем снимке ФС: регистронезависимое
...
/// Синтетический суффикс `::<other>` в `scope` срезается. Без снимка/скана — пусто.
#[tauri::command]
