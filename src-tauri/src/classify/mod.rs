...
//! правил очистки (`rules.rs`, пост-проход по дереву). Данные-таблицы — в
//! `data.rs`, логика — здесь и в `rules.rs`.
//!
//! Категория присваивается по расширению; неизвестное → `Other`. Папки сами
//! по себе категории не несут (`Other`); кандидаты на очистку размечает
//! `rules::apply_cleanup`.
...
/// файлы — по расширению, неизвестное расширение → `Other`.
...
/// База давности — `mtime` (atime недостоверен). Папки сюда не попадают.
