...
//!
//! Назначение (docs §5): быстрое переоткрытие без рескана и основа для diff
//! между снимками (фаза 3). Храним один — последний — снимок: дерево
//! сериализуется плоско (арена + ссылка на родителя), при загрузке
//! восстанавливаются списки детей и индекс путей.
//!
