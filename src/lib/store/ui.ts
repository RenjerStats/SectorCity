/**
 * Сторы «оболочки» UI (header / footer / 3D-viewport).
 *
 * Композиция: сверху Header, по центру 3D-сцена, снизу Footer. Footer —
 ...
 * канал `uiCommand`): Header не дёргает 3D/IPC напрямую.
 */
...
export type UiCommand =
  ...
  | { kind: "cancel" }
  | { kind: "reset" }
  /** Прыжок по хлебной крошке. */
  | { kind: "goToCrumb"; index: number }
  /** Войти в режим «Сканер мусора». */
  | { kind: "enterCleanup" }
  ...
  | { kind: "deselect" }
  /** Действие над активным узлом с клавиатуры. «Активный» — выбранный
   *  (карточка), иначе наведённый; `drill`/`properties`/`mark` работают по
   *  наведённому. Реализует Scene (у него доступ к interaction/drill). */
  | { kind: "nodeAction"; action: NodeAction }
  /**
   * Навигация к узлу по пути (клик по результату поиска → «дойти» до здания).
   * Scene перестраивает мир на уровне-родителе узла и восстанавливает крошки
   * от корня; совпадение остаётся подсвеченным.
   */
 ...
 * стор, а не прямой вызов — держим канал «клавиатура → DOM» через стор.
 */
...
/**
 * Кратковременная всплывающая плашка-подтверждение. Нужна для действий без
 * иного видимого следа — прежде всего «копировать путь». `id` растёт на каждый
 * показ, чтобы компонент рестартовал таймер авто-скрытия даже при одинаковом
 * тексте подряд.
 */
...
/**
 * Сводка текущего уровня — питает дефолтный контент footer (полоса заполнения
 * по категориям + итоги). Композиция текущего уровня (его прямые дети), а не
 * всего диска. Пишет владелец сцены после загрузки уровня; читает Footer.
 */
