 ...
 * - `relative` — порог как доля объёма папки (применяется на каждом уровне,
 *   включая вложенные превью; у каждой папки свой масштаб);
 ...
 * Папки никогда не сворачиваются — только файлы. `topNCap` — потолок числа зданий
 * на уровень (страховка перфо), не основной контрол (0 — без потолка).
 */
