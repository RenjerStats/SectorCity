<script lang="ts">
  ...
  // Компас-«полярная звезда»: вращающаяся часть. Угол пишется
  // ИМПЕРАТИВНО покадрово (высокочастотное из камеры — в обход стора).
  let compassRotEl = $state<HTMLSpanElement | undefined>(undefined);
  // Контекстное меню ПКМ: узел под курсором + позиция, либо null.
  ...
  /**
   * Структурный фильтр: из РАСКЛАДКИ убираются узлы
  ...
  onMount(() => {
    ...
    function wireOverlays() {
      ...
      const offLod = handle.onFrame(() => {
        ...
        lastFrameT = now;
        nav?.updateLOD();
        nav?.updateFade(dt); // кросс-фейд градиента затемнения декора
      });
      // Позицию окон над зданием ставим покадрово императивно (вне реконсиляции
      // Svelte): полное — по якорю выбранного, мини — по наведённому.
      const place = (
    ...
    // Глобальный поиск: дебаунс-запрос к снимку → список в footer.
    ...
    // Скрытые узлы — структурный фильтр: пересобираем город при
    ...
    // миры общаются только через стор. Снимаем команду ДО исполнения,
    ...
    //
    // Снимок бэк читает В ФОНЕ: `currentRoot` может ответить
    ...
    async function runStartFlow() {
       ...
       *  демо + приветствие), прекомпилировать шейдеры и снять оверлей. */
      async function startWith(root: string | null): Promise<void> {
        if (started) return;
        started = true;
        try {
          const target = root ?? "";
          // Замер: первый buildCity + compileAsync + первый кадр города —
          ...
          // иначе их синхронная сборка на первом кадре города даёт jank.
  ...
  async function scanFolder() {
    ...
    try {
      ...
      if (completed) {
        ...
        // снятия прогресс-панели, чтобы первый кадр не собирал их синхронно.
        await compileShaders();
  ...
  /** После бесшовного `up` дочитать ОДИН дальний декор-слой, если буфер контекста
   ...
   * fire-and-forget — твин камеры не блокируем. */
  ...
  async function goToCrumb(index: number) {
    ...
    interaction?.refreshHover();
    // Бесшовный подъём мог оголить дальний контекст — дочитать его, не блокируя.
   ...
   *  наведённый — «правда под курсором/в фокусе». */
  function actionNode(): ScanNode | null {
    return selectedNode.get() ?? hoveredNode.get();
  }

  /** Диспетчер действий над узлом с клавиатуры. `drill`,
  ...
  /**
   * «Сделать эту папку корнем»: жёсткое переоткрытие текущего уровня
  ...
  /** Скрыть узел из визуализации (кнопка «Исключить» в карточке):
  ...
  /** Навигация к узлу по пути (клик по результату поиска → «дойти»).
...
<div class="scene">
  ...
  <!-- Компас-«полярная звезда»: показывает север карты, вращается с
  ...
  <!-- Единое окно над зданием: компактный hover-вид разворачивается по клику в
       полную карточку. Узел — выбранный, иначе наведённый; режим
  ...
  <!-- Контекстное меню ПКМ: рендерится Scene (владелец 3D), данные
