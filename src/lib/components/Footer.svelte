<script lang="ts">
   ...
   * Контент выбирается по приоритету:
   *
  ...
  // Активный поиск занимает footer списком результатов — приоритет
...
<footer class="footer">
  ...
  {:else if hidden}
    <HiddenPanel />
  {:else}
    <!-- Крошки слева (от начала чтения), справа — KPI+полоса заполнения
         одним блоком (число привязано к полосе); «Легенда» — краем. -->
    <div class="browse">
