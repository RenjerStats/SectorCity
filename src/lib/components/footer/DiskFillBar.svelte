<script lang="ts">
  /**
   * Дефолтный контент footer в обычном режиме: dot-matrix KPI-итог,
   * сегментная полоса «заполнения» уровня по категориям, подпись
   * категории-лидера и мини-легенда. Цвета сегментов = цвета
...
<div class="fill">
  {#if summary && summary.totalBytes > 0}
    <!-- KPI-итог привязан к полосе (число описывает именно её). -->
    ...
    <!-- Подпись категории-лидера: когда доминирует одна категория,
