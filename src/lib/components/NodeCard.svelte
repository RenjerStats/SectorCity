<script lang="ts">
   ...
   * ПОЗИЦИЮ ставит владелец (Scene) императивно покадрово. Действия
  ...
  <div class="card" class:expanded role="dialog" aria-label="Карточка узла">
    ...
    <div class="row info-row">
      ...
      {:else}
        ...
        >
          <!-- Dot-глиф категории — избыточный к цвету канал. -->
...
<style>
  ...
  /* Кандидат на очистку — семантический амбер (--stale), не акцент:
     причина + уверенность + объяснение словами. */
  .cleanup-warning {
    display: flex;
    align-items: flex-start;
    gap: 0.4rem;
