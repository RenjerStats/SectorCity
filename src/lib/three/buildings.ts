/**
 * Уникальная геометрия зданий по категории файла.
 *
...
function glass(): Material {
  return quality.active.pbr
    ? new MeshPhysicalMaterial({
        ...
        // Почти-зеркальная панель на тонких «перегородках» даёт субпиксельные
        // искры-пилу на дистанции; чуть повышенная roughness убирает мерцание.
        roughness: 0.14,
        metalness: 0,
        // Полная сила env-отражения на маленькой панели выжигает бОльшую часть
        // площади таким же бликом, как у купола — интенсивность снижена.
        envMapIntensity: 0.6,
        clearcoat: 0.5,
...
function steel(): Material {
  // metalness/roughness смягчены: чистый металл не имеет диффуза — тени на
  // обвязке/крышках видны только в зеркальном лобе солнца, а острый спекуляр
  // на тонких деталях «искрил пилой» на дистанции.
  return quality.active.pbr
...
/**
 * `code` — «терминал с кодом»: единый мотив СТРОК на крыше и на экране фасада.
 * Крыша — синяя плита с тёмными продольными полосками-«строками»; на фасаде —
 * глянцевый чёрный ЭКРАН, по которому идут яркие синие строки (код на терминале).
 */
function makeCode(): BuildingDef {
  // Синий канал крыши: плита + яркие строки на экране фасада (одна группа-материал).
  const blue = part([
    crownSlab(BODY_TOP, 0.46),
    rbox(0.5, 0.035, 0.05, -0.04, 0.16, 0.5, 0.012), // строки на экране (разной длины
    rbox(0.4, 0.035, 0.05, -0.09, 0.02, 0.5, 0.012), //   и со сдвигом — как текст кода)
    rbox(0.46, 0.035, 0.05, -0.06, -0.12, 0.5, 0.012),
  ]);
  // Тёмные продольные полоски-«строки» на крыше (контраст, не сливаются с синим).
  const stripes = part([
    rbox(0.16, 0.05, 0.9, -0.28, 0.475, 0, 0.02),
    rbox(0.16, 0.05, 0.9, 0.0, 0.475, 0, 0.02),
    rbox(0.16, 0.05, 0.9, 0.28, 0.475, 0, 0.02),
  ]);
  // Экран-терминал на переднем фасаде — глянцевая чёрная панель (в пределах габарита).
  const screen = rbox(0.66, 0.6, 0.04, 0, -0.04, 0.5, 0.03);
  return assemble([
    { geo: bodyGeo(), mat: plastic(BODY_TINT.code), color: BODY_TINT.code },
    { geo: screen, mat: glass(), color: GLASS_PANEL_COLOR },
    { geo: blue, mat: crownMat("code"), color: CATEGORY_COLOR.code },
