 ...
 * детализация по глубине и фикс окклюзии декора.
 *
...
describe("buildLevel: купола (+1) и детализация по глубине", () => {
  ...
  it("пикинг: купол → drill в папку, файл верхнего уровня → select (drillTarget = он сам)", () => {
    ...
    // isPickTarget: купол и файл верхнего уровня — самостоятельные цели; вложенный
    // превью-домик под куполом «прозрачен» для пикинга (курсор упирается в купол).
    expect(level.view.isPickTarget(dome, idxA!)).toBe(true);
    expect(level.view.isPickTarget(c.mesh, c.index)).toBe(true);
    expect(level.view.isPickTarget(a1.mesh, a1.index)).toBe(false);
