/**
 * Числовое зеркало тем для 3D-слоя: Three.js не читает CSS-переменные, поэтому
 * DOM берёт цвета из themes.css, а 3D — те же цвета отсюда, синхронно.
 *
 * Рантайм-свап материалов сцены (setColor/dispose при смене темы) ещё не
 * подключён — сейчас 3D-слой красится под дефолт (graphite-red).
 */
