 ...
 * под `G⁻¹` опускается на стопку плит ступенями вниз — «слоёный пирог».
 ...
 * («во сколько раз контент слоя показан крупнее активного»).
 */
...
/**
 * Drill: композиция `G⁻¹` поверх слоя. Вошли в ребёнка с размещением
 ...
 * `cy` опускает его по Y (стопка плит вниз — слоёный пирог).
 */
...
/**
 * Up: композиция `G` поверх слоя — ровно обратное `composeInverse`.
...
/**
 * Трансформ ПРЕДКА из трансформа его ребёнка (`up`-дочит дальнего слоя).
