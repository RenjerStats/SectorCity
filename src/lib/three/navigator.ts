/**
 * Бесшовная навигация по уровням.
 *
 ...
 *     и при `up` — чтобы шов не рвался резким исчезновением окружения.
 *
 ...
 * пиксель-в-пиксель та же до и после — origin shift невидим.
 *
...
/**
 * Потолок числа хранимых декор-слоёв (`N_max`). Слои сверх него (самые дальние)
 * уничтожаются на drill; на `up` дальний слой добирается заново. Контекста хотим
 * ≥ `DECOR_KEEP_DEPTH` (3) — потолок чуть выше под запас. Каждый слой — отдельная
 * геометрия уровня, поэтому потолок ограничен памятью/перфом.
 */
export const N_MAX = 4;

/**
 * Порог `S0`: пока кумулятивный масштаб слоя `S_k ≤ S0` — он «ещё крупный»,
 * держим полный бюджет. Каждое удвоение `S_k` сверх `S0` (ещё «октава» зума-наружу)
 * срезает один уровень LOD.
 */
const DECOR_S0 = 1.5;
/**
 * Порог `S_drop`: за ним слой практически вне кадра (в центре — активный,
 * слой раздут ×`S_k`) → бюджет 0 (контур). Дальше он же — кандидат на дроп.
 */
const DECOR_S_DROP = 7;

/**
 * Зум-зависимый LOD-бюджет декор-слоя по его `S_k`:
 ...
 * S_k) быстро срезает дальние — они и так ушли из кадра.
 */
...
/** Длительность кросс-фейда градиента/появления дальнего слоя. */
const DECOR_FADE_MS = 250;

/**
 * Множитель затемнения декор-слоя по его `S_k`: `DECOR_DIM_NEAR` у близких
...
interface DecorLayer {
  ...
  /** Кумулятивный масштаб `S_k = transform.scale`: во сколько раз контент слоя
   *  показан крупнее активного. Мера «удалённости на задний план». */
  S: number;
  /** Глубина LOD, до которой слой несёт реальную застройку. */
  budget: number;
  /** ПОКАЗАННЫЙ множитель затемнения; ведётся кросс-фейдом к
   *  `colorFactorFromS(S)`. На свопе НЕ трогается — отсюда инвариант кадра. */
  dimShown: number;
}
...
export interface CityNavigator {
  ...
  /**
   * Путь уровня, ЧЕЙ РОДИТЕЛЬ стоит дочитать дальним декор-слоем, если в стеке
   * есть место (< `N_MAX`). Это самый дальний удерживаемый слой (или активный,
   ...
   * построив его из `parentNodes` и пристыковав ПОДОБИЕМ к цепочке. Нужен,
  ...
  updateLOD(): void;
  /**
   * Покадрово: кросс-фейд затемнения декор-слоёв к их `colorFactorFromS(S)`.
   * `dtMs` — время кадра. Меняет только ПОКАЗАННЫЙ dim (не на свопе), поэтому
   * не рвёт инвариант кадра. No-op, если декора нет / всё уже сошлось.
   */
  ...
  /**
   * Облик режима «Сканер мусора». `view ≠ null` — включить «вид
...
export function createNavigator(handle: SceneHandle): CityNavigator {
   ...
   *  drill/up меняют `S`/позицию слоёв). Бюджет — по `S_k` (обратимое скрытие без
   *  ребилда). «Слоёный пирог» плит (верх плиты дрилл-района L1 на полу активного,
   *  дальше ступенями вниз) теперь даёт сам трансформ-подобие слоя: `G⁻¹` с
   *  `cy = contentFloor` опускает каждый слой на нужную высоту, плита стоит на
   *  каноническом месте (`setDecor` её больше не репозиционирует). */
   ...
   *  не уходят под землю активного. Это бесшовно: дальняя земля — та же
  ...
  async function drill(
    ...
    // синхронно с зумом — вместо «прыжка» исчезновения на свопе.
    ...
    levelSpans.set(node.path, childSpan); // для возможного дочита на `up`
    ...
    await handle.flyTo(
      ...
      () => {
        ...
        // (режим действует на всю сцену, а не только активную папку).
        ...
        // Стек сдвинулся вглубь — пересчитать LOD-бюджеты. Вертикальную стопку
        // плит даёт сам рибейз-подобие слоёв (`cy`), отдельный подиум не нужен.
        applyDecorBudgets();

        active = next;
        setActiveView(active);
        // Землю показываем у самого дальнего слоя (дно пирога). ВАЖНО: после
  ...
  async function up(ms: number): Promise<void> {
    ...
    // ется. Стартовое затемнение родителя — его текущий показанный
    ...
    await handle.flyTo(
      ...
      () => {
        ...
        // Стек поднялся на уровень — бюджеты выросли (скрытые инстансы возвращаются).
        // Стопку плит пересчитывать вручную не нужно: её несёт рибейз-подобие слоёв.
        applyDecorBudgets();
  ...
  function appendFarAncestor(
    ...
    // T_parent = T_child ∘ g⁻¹: продолжаем цепочку «ещё крупнее, дальше».
    const t = composeChildInverse(g, childTransform);
    const budget = budgetFromS(t.scale); // зум-зависимый бюджет по его `S_k`
    content.add(gp.group);
    applyLayerTransform(gp.group, t);
    // Силуэт ребёнка скрыт (его рисует свой слой), сразу со срезанным LOD-бюджетом.
    // Появляется из фона (dim=0) — `updateFade` проявит до `colorFactorFromS(S)`.
    ...
    // стопку плит несёт рибейз-подобие нового слоя (`cy`).
    applyDecorBudgets();
