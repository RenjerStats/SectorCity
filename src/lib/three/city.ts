/**
 * Построение «города» одного уровня как самостоятельной `Group`: папки — стеклянные
 * купола, детализация по глубине вместо дистанционного LOD.
 *
 ...
 * и делает drill дешёвым: размещение уровня в прямоугольник района — честная
 * similarity `G(local)=s·local+C` (см. `navigator.ts`), а промоут «купол →
 * активный уровень» — пиксель-в-пиксель по домикам (анимируется только снятие
 * стекла).
 *
 * Координаты d3 → мир — ТОЛЬКО через `layoutToWorld`.
 *
 * Сущности рендера:
 ...
 *
 * Детализация по ГЛУБИНЕ, не по расстоянию до камеры: прямые дети текущей папки
 * (+1) — купола с
 ...
 * не «переразъезжается». Дороги-бордюры упразднены: носитель dot-приёма — земля
 * целиком (`buildGroundDots`), разделение блоков несут постаменты и стекло.
 * Значение ниже — ЕДИНСТВЕННЫЙ
 ...
 * размываются transmission'ом и протыкают стекло, ломая буфер глубины).
 */
...
/**
 * Тинт-множители уверенности кандидата в режиме очистки: кандидат
 ...
 * верх, поэтому ничего не тонет в плите. У +1 — фикс `RING_HEIGHT`
...
const RING_RIM = 1.0;
/**
 * Купол охватывает свою застройку с запасом-«воздухом» сверху (высота самого
 * купола — производная, величину не кодирует). Footprint купола слегка
 ...
 * (протыкает буфер глубины). Поэтому footprint стекла считаем ПО ОСЯМ
...
/**
 * Анимация снятия купола при drill (и обратного «надевания» при up):
...
export interface Placement {
   ...
   * `G⁻¹` уходит на стопку плит вниз (слоёный пирог). См. `transform.ts`.
   */
  cy: number;
...
export interface CityView {
   ...
   * не расстоянием, поэтому здесь делать нечего — облик статичен.
   */
...
/**
 * Анимируемый «снятый» купол: отдельный полупрозрачный меш,
...
/**
 * Анимируемый «рой» куполов — расширение `DrillDome` на
...
export interface Level {
   ...
   * (слоёный пирог). Плита стоит на своём каноническом месте — нужный
   ...
   * инстансы глубиной d3 > `budget` скрываются; по умолчанию полный
   * (`PREVIEW_MAX_DEPTH`). `dim` — начальный множитель затемнения (по умолчанию
   * `DECOR_GRAY`).
   */
  setDecor(excludePath?: string, budget?: number, dim?: number): void;
  /**
   * Сменить LOD-бюджет уже-декорного уровня без пересборки: при drill/up
  ...
  /**
   * Покадрово сменить множитель затемнения декора: `1` — полный цвет,
  ...
  /**
   * Показать/скрыть землю-«пол» этого уровня. Навигатор держит видимой
  ...
  setActive(): void;
  /**
   * Снять купол района `path` в отдельный анимируемый меш: инстанс купола в
   * общем меше скрывается, возвращается ручка `DrillDome`, чей
  ...
  /**
   * Подсветка-фильтр: несовпадающие с предикатом узлы гасятся
  ...
  /**
   * Облик режима «Сканер мусора». При `view ≠ null` сцена уходит в
 ...
 * категорийный тинт допустим только здесь, на основании.
 */
 ...
 * настоящее матовое стекло: `transmission` + `roughness` морозят прошедший
...
/**
 * Синтезировать анимируемый «рой» куполов из снимков `descs`: отдельные
 * полупрозрачные меши в собственной `group`, которую навигатор размещает
 ...
 *   - `domeMesh`     — матовые стеклянные купола папок +1 (transmission);
...
export function buildLevel(
  ...
  group.add(ground);
  // Dot-grid земли — регулярная мировая сетка точек по всему апрону,
  ...
  if (districts.length > 0) {
    // +1 стекло — НАСТОЯЩЕЕ матовое (`transmission`): дешёвый
    // `transparent+opacity` физически НЕ размывает содержимое (только гасит
    ...
    districts.forEach((d, j) => {
      ...
      // Пикинг папки: за один шаг ходим только на уровень глубже →
      ...
      if (d.d3depth === 1) {
        placements.set(d.node.path, {
          ...
          // (пол 0) садится сюда → drill бесшовен по Y (decor уезжает вниз).
          cy: d.contentFloor,
          cz: full.centerZ,
          w: full.width,
          d: full.depth,
  ...
  // по глубине: в декоре инстансы глубже бюджета скрыты, в активном — полный.
  let isDecor = false;
  let excludedIdx: number | null = null;
  let decorBudget = PREVIEW_MAX_DEPTH;
  // Множитель затемнения декора: 1 — полный цвет (неотличим от активного),
  ...
  const view: CityView = {
    updateLOD() {
      // Детализация по глубине статична — покадрово делать нечего.
    },
    pickMeshes() {
      if (isDecor) return []; // декор не кликабелен
   ...
   *  прозрачности («плавно к серому/в фон», а не alpha). */
  ...
  function applyDecor(): void {
    ...
    for (const g of buildGroups) {
      g.items.forEach((it, i) => {
        // Скрыто, если в исключённом поддереве ЛИБО глубже LOD-бюджета.
        const hide =
    ...
    if (domeMesh && baseMesh && baseMatteMesh) {
      ...
      districts.forEach((d, j) => {
        // Сам дрилл-район: купол и застройку скрываем (их несёт активный
        ...
        // LOD-срез: район глубже бюджета тоже скрыт (обратимо).
...
/**
 * Dot-grid земли: регулярная мировая сетка точек по ВСЕМУ апрону, включая площадь
 * под городом (в зазорах между зданиями читается как «текстура земли», под домами
 * скрыта их непрозрачностью). Яркость точки — bright в центре, радиально гаснет к
 * краям (та же кривая `groundFade`, что и у самой земли), поэтому сетка тает в фон
 * вместе с апроном. Дорог-бордюров нет: разделение блоков несут постаменты и
 * стекло куполов.
 */
