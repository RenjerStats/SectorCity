/**
 * Единственный глобальный обработчик клавиатуры (монтируется один раз в
 * Hotkeys.svelte). DOM/оверлейные сторы дёргаем напрямую, а 3D/навигацию —
 * через `uiCommand`, который исполняет Scene.
 *
 * Правила:
 *  - Одиночные клавиши (без Ctrl/Alt/Meta) не срабатывают при фокусе в текстовом
 *    поле (иначе буквы уходят в строку поиска) — см. `isTyping`.
 *  - Ctrl-комбинации работают всегда, кроме Ctrl+C при фокусе в поле (обычное
 *    копирование текста).
 *  - Esc — «шаг назад» стеком; модалки/меню сами обрабатывают свой Esc первыми,
 *    поэтому при открытом оверлее центральный обработчик не вмешивается.
 */
import {
  dispatchCommand,
  contextMenuOpen,
...
function onKey(e: KeyboardEvent): void {
  ...
  // Цифры 1..8 (верхний ряд или numpad) — тумблеры категорий, порядок = ALL_CATEGORIES.
  const digit =
